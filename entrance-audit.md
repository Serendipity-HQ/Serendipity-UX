...
2. **Closed dispatch entrance — healthy after fix.** One large, clearly labeled button communicates that the week's recommendations are enclosed and gives a single obvious action. Evidence: `.design-qa/entrance-modernization/04-implementation-closed-desktop.png`, `.design-qa/entrance-modernization/06-implementation-closed-mobile.png`.
3. **Opened recommendation list — healthy after fix.** Every available recommendation appears together with lane, title, host, personalized reason, date, price state, and detail affordance. Evidence: `.design-qa/entrance-modernization/05-implementation-open-desktop.png`, `.design-qa/entrance-modernization/07-implementation-open-mobile.png`.
...
- Dynamic count language maintains trust when the quality gates return fewer than three matches.
...
- Removed contradictory “three” messaging for a two-result state.
